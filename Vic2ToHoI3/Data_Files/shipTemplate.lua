...
function P.TechWeights(voTechnologyData)
	...
	if (voTechnologyData.PortsTotal > 0) then
		...
		elseif (voTechnologyData.icTotal <= 20) then
			laArray= {
			0.25, -- _RESEARCH_LAND_
			0.14, -- _RESEARCH_LAND_DOC_
			0.12, -- _RESEARCH_AIR_
			0.10, -- _RESEARCH_AIR_DOC_
			0.14, -- _RESEARCH_NAVAL_
		 ...
		 laArray= {
			0.18, -- _RESEARCH_LAND_
			0.13, -- _RESEARCH_LAND_DOC_
			0.12, -- _RESEARCH_AIR_
			0.10, -- _RESEARCH_AIR_DOC_
			0.18, -- _RESEARCH_NAVAL_
			0.14, -- _RESEARCH_NAVAL_DOC_
			0.13, -- _RESEARCH_INDUSTRIAL_
			0.01, -- _RESEARCH_SECRET_
			0.01}; -- _RESEARCH_UNKNOWN_
		
		end 
	else --default land why the *** I used major here?
		if (voTechnologyData.IsMajor) then	
			laArray= {	
			0.28, -- _RESEARCH_LAND_
			0.15, -- _RESEARCH_LAND_DOC_
			0.18, -- _RESEARCH_AIR_
			0.13, -- _RESEARCH_AIR_DOC_
			0.03, -- _RESEARCH_NAVAL_
			0.03, -- _RESEARCH_NAVAL_DOC_
			0.15, -- _RESEARCH_INDUSTRIAL_
			0.02, -- _RESEARCH_SECRET_
			0.03}; -- _RESEARCH_UNKNOWN_
			
		elseif (voTechnologyData.icTotal >= 20) then
			laArray= {
			0.33, -- _RESEARCH_LAND_
			0.17, -- _RESEARCH_LAND_DOC_
			0.17, -- _RESEARCH_AIR_
			0.14, -- _RESEARCH_AIR_DOC_
			0.01, -- _RESEARCH_NAVAL_
...
function P.NavalTechs(voTechnologyData)
	...
	preferTech = {
		...
		"destroyer_engine",
		"carrier_technology"};
	 elseif(voTechnologyData.icTotal < 75) then --antiarcraft to 2 beacuse the starting units and becasue AA are upgradable
	 	 	ignoreTech = {
		{"lightcruiser_technology", 0},
		{"lightcruiser_armament", 0},
...
function P.OtherTechs(voTechnologyData)
	local ignoreTech = {
		...
		{"electornicegineering_research", 1},
...
function P.AirRatio(voProductionData)
	...
	local laArray
	--TODO RANDOMHOI add strategic for > 150 IC
	if (voProductionData.icTotal ) < 20 then
			laArray = {
			interceptor = 1,
			multi_role = 0,
