...
namespace parser_generic{
	...
	*/
	shared_ptr<Object> parse(const std::string &file_path, Encoding file_encoding);

	/*
	Parses the file pointed to by the path, assuming UTF-8 encoding
	*/
	shared_ptr<Object> parseUTF_8(const std::string &file_path);
	
	/*
	Parses the file pointed to by the path, assuming ISO 8859 15 encoding
	*/
	shared_ptr<Object> parseISO_8859_15(const std::string &path);
